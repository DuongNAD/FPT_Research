...
def analyze_syscalls(log_path: str) -> dict:
    ...
    try:
        ...
        if GEMINI_API_KEY == "dummy_key" or genai is None:
            logger.warning("Mock LLM enabled. Trả về kết quả phân tích giả định.")
            is_malware = "fake" in log_path.lower()
            return {
                "suspicious_activities": ["Cố gắng đọc file /etc/passwd"] if is_malware else [],
                "network_connections": ["192.168.1.100:4444"] if is_malware else [],
                "files_accessed": ["/etc/passwd"] if is_malware else ["/tmp/normal"],
                "severity_score": 9 if is_malware else 1,
                "summary": "Mã độc nhắm vào C2." if is_malware else "Gói tin chuẩn an toàn."
            }
